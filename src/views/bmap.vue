				...
				<baidu-map class="bm-view" ak="UN65YsvjLf7Lu85y3RKQtA7uaceLtPye" :pinch-to-zoom="true" :center="nearby.center"
				 :zoom="15" @ready="initBmap">
					...
					<bm-marker v-if="geolocationOK" :position="bm_marker.point" :icon="bm_marker.icon">
						<bm-label :content="bm_label.content" :labelStyle="{ color: 'red', fontSize: '14px' }" :offset="{ width: -80, height: 10 }" />
					</bm-marker>
					<!-- 自驾 -->
					<bm-driving v-if="geolocationOK" :start="nearby.center" :end="bm_marker.point" :auto-viewport="true"></bm-driving>
					<!-- 自驾调整视窗导致信息窗体失效 -->
